...
#define CUDALAUNCH(funcname, ...)                                                \
    if (profiler_on)  gettimeofday(&time_start, NULL);                           \
...
typedef struct cell_info
{
    ...
    cell_info(int in_x_extra, int in_y_extra,
...
#define PACK_ARGS                                         \
    ...
    int getBufferSize(int edge, int depth, int x_inc, int y_inc);

    void unpackBuffer(const int which_array,
                      const int which_side,
                      double *buffer,
                      const int buffer_size,
                      const int depth);

    void packBuffer(const int which_array,
                    const int which_side,
                    double *buffer,
                    const int buffer_size,
                    const int depth);

    void errorHandler(int line_num, const char *file);

    void update_array(int x_min, int x_max, int y_min, int y_max,
...
#define DIE(...) cloverDie(__LINE__, __FILE__, __VA_ARGS__)
    void cloverDie(int line, const char *filename, const char *format, ...);
...
public:
    ...
    std::vector<double> dumpArray(const std::string &arr_name, int x_extra, int y_extra);
    ...
    void packRect(double *host_buffer, dir_t direction,
    ...
    CloverleafCudaChunk(INITIALISE_ARGS);

    CloverleafCudaChunk(void);
    ~CloverleafCudaChunk(void);
};
