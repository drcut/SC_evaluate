...
extern "C" void update_halo_kernel_cuda_(const int *chunk_neighbours,
                                         const int *fields,
                                         const int *depth)
{
...
void CloverleafCudaChunk::update_array(int x_min, int x_max, int y_min, int y_max,
...
#define CHECK_LAUNCH(face, dir)                                                                                                                                                                                                                                  \
    ...
    {                                                                                                                                                                                                                                                            \
        const int launch_sz = (ceil((dir##_max + 5 + grid_type.dir##_extra) / static_cast<float>(BLOCK_SZ))) * depth;                                                                                                                                            \
        device_update_halo_kernel_##face##_cuda<<<launch_sz, BLOCK_SZ>>>(x_min, x_max, y_min, y_max, grid_type.x_extra, grid_type.y_extra, grid_type.x_invert, grid_type.y_invert, grid_type.x_face, grid_type.y_face, grid_type.grid_type, cur_array_d, depth); \
...
void CloverleafCudaChunk::update_halo_kernel(const int *fields,
                                             const int depth,
                                             const int *chunk_neighbours)
{
